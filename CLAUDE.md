...
- **调试守则**: 遇到报错或功能异常时，先查 skills，再猜原因，按以下顺序：
  1. 先读 `skills/backend/references/core/debugging-guide.md`（后端/环境/部署/Token/冒烟测试问题）
     或 `skills/frontend/references/iidp-frontend-dev-manual/iidpDoc/03.前端开发手册/015.效率与调试/`（前端问题）
  2. 对照症状速查表，定位应读的具体文件
  3. skills 中未覆盖时，再结合日志和代码分析
  4. 问题解决后，将新发现的问题模式追加回 `debugging-guide.md`（知识回流）
