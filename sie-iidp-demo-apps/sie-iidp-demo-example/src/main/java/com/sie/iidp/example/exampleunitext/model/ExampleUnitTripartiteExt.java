...
@Setter
@Slf4j
@Model(tableName = "example_unit_tripartite_ext", name = "example_unit_tripartite_ext", description = "Example-单位-第三方-扩展", displayName = "Example-单位-第三方-扩展", isLogicDelete = Bool.False, isAutoLog = Bool.True,parent = {"example_unit"})
public class ExampleUnitTripartiteExt extends BaseModel<ExampleUnitTripartiteExt> {
    ...
    public boolean enableDisable(RecordSet rs, String statusFlag) {
        ...
        if (recordSet.any()) {
            ...
            valMap.put(ExampleUnitTripartiteExt.F_IS_ENABLE, statusFlag);
     ...
     */
    @Override
    public List<ExampleUnitTripartiteExt> search(Filter filter, List<String> properties, Integer limit, Integer offset, String order) {
        try {
            this.getMeta().getArguments().put(AttributesConstant.useDisplayForModel, false);
            Filter.FilterOp timeRangeOp = filter.getFilterOp(ExampleUnitTripartiteExt.F_TIME_RANGE);
