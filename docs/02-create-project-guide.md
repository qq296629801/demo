...
**用途**：解析 `spec/09-ui/` 目录下的静态 HTML 原型，使用 Playwright MCP 提取结构化交互需求，生成 `interaction-spec.md`（8 节格式）。
...
A：在 `code-index` 生成了 `spec/09-ui/*.html` 之后，`/sdd-spec` 前后均可运行。建议在 `/sdd-contracts` 之后运行，这样 API 映射（§7）才能引用到 `contracts.md` 中的接口。
