...
| 全新 IIDP 工程、从零搭建、POC / 原型 | `references/sdd-greenfield.md` |
...
- 前端规格（`frontend-spec.md`，每个 feature 必须生成）：实现分支判断、节点树、selector、节点 id、数据源、绑定、事件和 commands；标准模板/在线视图无需前端代码时，也必须在 §9 记录原因。模板来自 `sdd-frontend.md`，步骤见 `sdd-workflow.md` Step 1.5b。
...
| 当前任务 | 进入子 skill |
|---|---|
| 有业务需求/用户故事，需要生成前端规格文档 | `skills/frontend/references/iidp-frontend-spec-doc/SKILL.md` |
| 已有前端规格文档，需要生成前端工程代码 | `skills/frontend/references/iidp-frontend-spec-code/SKILL.md` |
| 需要创建前端工程或扩展应用 | `skills/frontend/references/iidp-frontend-init/SKILL.md` |
| 在已有应用中开发扩展视图、hook、数据源、自定义组件 | `skills/frontend/references/iidp-frontend-extension-dev/SKILL.md` |
| 查阅框架机制、组件属性、扩展协议 | `skills/frontend/references/iidp-frontend-dev-manual/SKILL.md` |
| 查阅标准模板节点 id 命名规则 | `skills/frontend/references/iidp-frontend-standard-ids/SKILL.md` |
...
**SDD 产物完成后必须进入代码实现**：`backend-spec.md`、`frontend-spec.md`、`plan.md`、`tasks.md` 全部产出后，按 `references/sdd-workflow.md` Step 4 Implement 执行代码生成。后端以 `backend-spec.md` 和 `tasks.md` 为输入，通过 `skills/backend/SKILL.md` 生成 Java 模型/服务/视图/菜单代码；前端以 `frontend-spec.md` §9 实现分支判断为准，需前端代码时通过 `skills/frontend/SKILL.md` 路由到对应子 skill 生成代码（标准模板页面跳过）。一次只完成 `tasks.md` 中一个未完成任务，完成即勾选。
