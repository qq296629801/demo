...
1. **读取规格文件，建立现状快照**：在做任何修改前，必须先完整读取所有将要更新的规格文件。对每个目标文件执行以下操作，**不得跳过**：

   a. **读取文件全文**（`backend-spec.md`、`frontend-spec.md`、`contracts.md`，按实际需要选择）。

   b. **提取现有条目清单**，记录以下信息用于后续比对：
      - `backend-spec.md §3`：当前字段表中已有的字段名列表
      - `backend-spec.md §4`：当前服务表中已有的服务名列表
      - `backend-spec.md §5`：当前视图 key、菜单 key、按钮 id 列表
      - `contracts.md`：已登记的服务 name、args、权限码
      - `frontend-spec.md`：已有的节点 id、ds_config name 列表

   c. **识别人工标注**：扫描文件中的 `待确认` 标记，记录位置，后续更新时不得覆盖这些标注，只能在其旁边补充信息。

   > 若规格文件不存在，标注"⚠ 待补充：规格文件缺失，请先运行 `/sdd-spec` 生成完整规格"，该文件的同步步骤跳过。

2. **逐文件拉取 diff**：对每个受影响代码文件，优先执行 `git diff $(git merge-base HEAD origin/...) HEAD -- <file>`，回退时用 `git diff HEAD -- <file>`。

3. **语义分析**：识别 diff 中变化的条目，与步骤 1 的现状快照比对，分为三类：
   - **新增**：代码中新出现、规格中不存在的字段/服务/节点 → 追加到对应章节末尾
   - **修改**：代码中已变更、规格中有对应条目 → 定位到该行/表格行，原地更新
   - **删除**：代码中已移除、规格中仍存在 → 标注删除线或加注"⚠ 代码已移除，待确认是否同步删除规格条目"，不自动删除
   - 识别范围——后端：字段名/类型/注解、服务签名/入参出参、视图 key、菜单 key、权限码；前端：节点 id、ds_config、bind_on_、commands、hook 路径

4. **最小外科式更新**：
   - 只操作步骤 3 分类后的具体行/表格行，**不重写整个章节，不触碰无关行**
   - 新增条目追加在对应表格最后一行，保持原有表格格式
   - 修改条目在原行原地替换，保留行号上下文
