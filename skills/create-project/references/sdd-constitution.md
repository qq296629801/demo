...
**建立项目宪法前，必须先读取 `skills/backend/references/core/platform-standards.md`**，将其中的强制约束写入 `iidp-stack.md`，作为整个项目的编码基线：

| 约束类别 | 来源章节 | 写入 iidp-stack.md 的内容 |
|---|---|---|
| 命名规范 | §命名规范 | appName、appPkg、模型名、字段名、常量命名规则 |
| Java 代码规范 | §代码规范 | 禁止手动声明审计字段、camelCase 严格区分大小写、禁止 Spring Boot 原生写法 |
| 模型设计规范 | §模型设计 | OneToMany/ManyToOne/ManyToMany 字段名/目标模型/级联策略必须完整 |
| 异常与事务规范 | §异常处理 | ModelException vs ValidationException 场景划分、IIDP 请求级事务约定 |
| SQL 访问规范 | §ER 查询 | 禁止 N+1、原生 SQL 使用边界 |

> **iidp-stack.md 中的平台规范约束不受项目定制影响**：即使项目覆盖了 `specs/templates/iidp-stack.md`，上表中的强制约束仍必须保留，不得降低或绕过。
