...
2. **契约先行**：模型名、视图 key、菜单 key、服务名、权限码、数据源名、节点 id 等必须在生成 backend-spec 和 frontend-spec 之前，先在 `integration-map.md`（架构级）和 `sdd-contracts.md`（API 级）中写清楚（对应 sdd-workflow.md Step 1.3a，强制步骤）。填写时必须参考 `skills/backend/references/core/` 对应文件，不得凭记忆填写注解参数、字段类型或 JSON 结构。
...
└── features/
    └── phase1-[feature]/
        ├── requirements.md
        ├── sdd-contracts.md    # API 级契约（属性/ER字段/服务签名/视图key，Step 1.3a 生成）
        ├── backend-spec.md
        ├── frontend-spec.md
        ├── plan.md
        ├── tasks.md
