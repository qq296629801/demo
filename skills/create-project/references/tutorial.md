...
```
workflow 文件：skills/create-project/workflows/iidp-sdd.yml
```
...
| 循环 | 类型 | 条件 | 上限 |
|---|---|---|---|
| Clarify 反复澄清 | do-while | 用户选 `more` | 5 轮 |
| Critique 修改重批 | do-while | 用户选 `revise` | 3 轮 |
| Implement 逐任务 | do-while | tasks.md 有未完成任务 | 100 次 |
| Validate 修复重验 | do-while | 验收有失败项 | 3 轮 |
| Spec 并行生成 | fan-out | backend + frontend | 并发 2 |
| 深度审查 5 Agent | fan-out | 5 个审查视角 | 并发 5 |

### 人工门控节点

| 门控 | 选项 | 拒绝行为 |
|---|---|---|
| Plan Review | approve / reject | abort（终止） |
| Blueprint 确认 | approve / reject | abort |
| Validate 失败 | fix / accept | skip（继续） |
| Phase 结束 | next-phase / done | skip（自然结束） |
