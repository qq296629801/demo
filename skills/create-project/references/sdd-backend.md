...
- **字段名大小写严格一致**：`@Property`、`set()`、`getStr()`、`getInt()` 等 Map 操作中的属性名字符串，必须与 Java `private` 字段声明完全相同（区分大小写）。
  ...
  | 字段声明 | ✅ 正确引用 | ❌ 错误写法 |
  |---|---|---|
  | `private String subClass;` | `getStr("subClass")` | `getStr("subclass")` / `getStr("SubClass")` |
  | `private Integer sortOrder;` | `getInt("sortOrder")` / `set("sortOrder", n)` | `getInt("sort_order")` / `set("sortorder", n)` |
  | `private String categoryId;` | `getStr("categoryId")` | `getStr("category_id")` |
...
建议拆分（在说明列标注"建议拆分到 service 层，待确认"）：

| 触发条件 |
|---|
| 预估方法体 21–40 行且涉及 ≥ 2 个模型 |
| 预估方法体 21–40 行且涉及 1 个模型 |
| 涉及 2 个模型且无其他强制触发 |

保留模型类：预估方法体 ≤ 20 行、仅涉及 1 个模型、无强制触发条件。
