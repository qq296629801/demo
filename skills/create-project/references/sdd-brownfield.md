...
基于步骤 1 的侦察报告，生成 `specs/integration-map.md`，作为工程的全局契约视图。

集成地图 Prompt：

```text
基于侦察报告，生成 specs/integration-map.md：

1. 填写 App 清单表格（appName、appPkg、主要模型、主要服务、前端扩展入口）。
2. 列出关键约束：
   - 已占用的节点 id（新增时不得冲突）
   - 已占用的权限码（新增时不得重复）
   - 跨 App 共享的数据源名称
   - 跨 App 的服务引用关系
3. 列出技术债概览（标准偏离和自定义技术债，来自侦察初步风险）。
4. 推断项标注「待确认」。

不补全整个存量系统的所有细节，只记录对新功能开发有约束意义的事实。
```
