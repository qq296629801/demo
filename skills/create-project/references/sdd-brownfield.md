...
步骤 1：侦察现有工程
步骤 2：初始化 SDD 基础设施
步骤 3：生成集成地图（integration-map.md）
步骤 4：选择产物生成策略
步骤 5：反向提取现有功能规格（可选）
  └─ 步骤 5a：验证反向提取结论（可选但推荐）
步骤 6：编写新功能规格
步骤 7：集成感知实施
  ├─ 步骤 7a：制定集成计划
  ├─ 步骤 7b：拆解实施任务
...
按选定的侦察深度（快速/标准/深度）执行证据采集，产出侦察报告。

侦察 Prompt：

```text
对当前 IIDP 工程做存量侦察，侦察深度：[快速 / 标准 / 深度]。

输出侦察报告，包含：
1. 工程结构：POM 聚合模块、启动模块、apps.json 登记的 App 清单。
2. App 契约概览：每个 App 的 app.json 路径、views 数量、menus 数量。
3. 模型与服务：@Model 类、@MethodService 方法数量及权限码分布。
4. 前端扩展：apps/<appName>/ 目录结构、selector 值清单。
5. 初步风险：命名冲突、未登记资源、无 auth 的写服务、非标准视图。
...
- App 数量：X 个
- 模型总数：X 个
- 服务总数：X 个（其中无 auth 写服务：X 个）
- 前端扩展文件：X 个

### App 清单
| appName | appPkg | 模型数 | 服务数 | 视图数 | 前端扩展 |
|---|---|---|---|---|---|
...
在工程根目录建立 `specs/` 目录结构，不改动任何现有代码：

```bash
mkdir -p specs/legacy
touch specs/integration-map.md
touch specs/tech-debt.md
touch specs/decisions.md
```
...
```text
specs/
├── integration-map.md        # 集成地图（工程全局视图）
├── tech-debt.md              # 技术债登记
├── decisions.md              # 关键接入决策记录
└── legacy/                   # 反向提取的存量功能规格
...
**目的**：基于步骤 1 的侦察报告，生成 `specs/integration-map.md`，作为工程的全局契约视图，供后续所有步骤引用。

**方法**：交互式生成（AI 提案 → 用户确认 → AI 定稿）

集成地图 Prompt：

```text
基于侦察报告，提案 specs/integration-map.md 的草稿内容，逐节向我确认：

1. App 清单：列出每个 App 的 appName、appPkg、主要模型、主要服务、前端扩展入口。
   请确认：App 清单是否完整？是否有未在 apps.json 登记但实际在运行的 App？
...
3. 技术债概览：列出侦察发现的标准偏离和自定义技术债条目。
   请确认：优先级判断是否准确？是否有已知但未在代码中体现的技术债？

推断项标注「待确认」。每节确认后再生成下一节，不要一次性输出全部内容。
```

**交互示例**：

```text
AI：基于侦察报告，我识别到以下 3 个 App：
    - OrderApp（appPkg: com.example.order）：2 个模型，5 个服务
    - InventoryApp（appPkg: com.example.inventory）：1 个模型，3 个服务
    - UserApp（appPkg: com.example.user）：1 个模型，2 个服务
    请确认 App 清单是否完整？是否有遗漏？[Y / 补充说明]

用户：清单正确，但还有一个 ReportApp 没在 apps.json 里，是历史遗留，不用纳入。

AI：了解，ReportApp 不纳入集成地图，在技术债概览中标注为「未登记遗留 App，
    建议评估是否下线或补充注册」。接下来确认关键约束……
```
...
### 关键决策
1. **[AppName] 的 [ModelName] 模型为跨 App 共享模型** — 任何 App 修改该模型需同步更新依赖方规格
2. **节点 id [xxx] 已被 [AppName] 占用** — 新 App 不得使用此 id，已在关键约束中标注
3. **[数据源名] 由 [AppName] 和 [AppName2] 共享** — 修改数据源配置需两侧 App 同步验证

### 产出文件
- `specs/integration-map.md`：App 清单（X 个）、关键约束（X 条）、技术债概览（X 条）

### 待确认项
- [推断项 1]（来源：侦察报告第 X 条）
- [推断项 2]（来源：apps.json 配置推断）

### 需要关注
- **技术债 P0 项**：[描述]，建议在进入实施前处理
- **推断项较多**：请在步骤 5a 验证阶段与实际运行环境核对

### 后续选项
A) 进入步骤 4，选择产物生成策略
B) 细化某个 App 的集成地图（补充更多模型/服务细节）
C) 先处理 P0 技术债再继续
```

### 步骤 4：选择产物生成策略

根据接入目标和工程规模，从以下四种模式中选一种（详见"产物生成策略"章节）：
...
选择标准模式、完整模式或混合模式时执行。对需要变更或依赖的现有 App 做 EDCR 分析（详见"EDCR 能力发现框架"章节），在 `specs/legacy/` 下生成业务规则文档。

业务规则提取 Prompt（见"业务规则提取 Prompt"章节）。

#### 步骤 5a：验证反向提取结论（可选但推荐）

在进入步骤 6 之前，验证步骤 5 提取的结论是否准确：

验证 Prompt：

```text
对 specs/legacy/<module>-business-rules.md 中的反向提取结论做验证：

1. 准确性：领域地图中的模型、服务、视图 key、权限码是否与代码一致？列出不一致项。
2. 完整性：是否覆盖了变更范围内的所有 @Model 和 @MethodService？列出遗漏项。
3. 推断确认：标注「待确认」的节点 id、权限码、数据源名，是否已与实际运行环境核对？
4. 风险登记：P0 条目是否已有处理计划？
...
基于集成地图和（可选的）存量规格，为本次变更编写规格文档。只覆盖变更范围，不补全整个存量系统。

新功能规格 Prompt：

```text
基于 specs/integration-map.md 和步骤 5 的存量规格（如有），为以下新功能编写规格：

功能描述：[描述新增的 App / 功能点]

规格文档（specs/<feature>/requirements.md）需包含：
1. 功能需求：用户流程、业务规则、状态流转。
2. 后端契约：新增 @Model、@MethodService、auth 权限码清单。
3. 前端契约：新增视图 key、菜单 key、selector、hook 和数据源。
4. 集成约束：依赖的现有服务、共享数据源、不得冲突的节点 id 和权限码。
5. 验收标准：可量化的验证条件。

推断项标注「待确认」，并在文档末尾汇总待确认清单。
```
...
在实施前，明确新功能与现有工程的集成点和修改范围：

集成计划 Prompt：

```text
基于 specs/<feature>/requirements.md 和 specs/integration-map.md，制定集成计划：

1. 集成点清单：新功能依赖的现有 App 服务、共享数据源、前端扩展节点。
2. 现有代码修改范围：哪些现有文件需要改动（app.json、menus.json、views/*.json、Java 类）。
3. 新增代码范围：需新建的文件和目录。
4. 实施顺序：考虑依赖关系的任务排序（如后端 @Model 先于 @MethodService，前端扩展后于后端服务）。
5. 风险与缓解：集成点的已知风险和缓解措施。
...
将集成计划拆解为可执行的原子任务，写入 `specs/<feature>/tasks.md`：

```markdown
## 任务清单

### 后端任务
- [ ] T-01 新增 @Model 类 [模型名]（影响文件：[路径]）
- [ ] T-02 新增 @MethodService [服务名]，auth="[权限码]"
- [ ] T-03 修改 app.json，添加新 view 配置
- [ ] T-04 更新 menus.json，添加菜单项

### 前端任务
- [ ] T-05 新建 apps/[appName]/views/[viewName].js
- [ ] T-06 配置 selector 和 hook

### 集成验证任务
- [ ] T-07 验证新权限码不与现有权限码冲突
- [ ] T-08 验证新节点 id 不与已占用节点 id 冲突
- [ ] T-09 验证跨 App 服务调用正常

### 修改现有代码的任务（标注影响范围）
- [ ] T-10 修改 [现有文件]，改动原因：[说明]
```

#### 步骤 7c：执行实施

按任务清单顺序实施，每完成一个阶段性任务后更新 `specs/integration-map.md`，记录新增的 App、服务、权限码和节点 id。实施完成后按 `specs/<feature>/validation.md` 执行验收。
