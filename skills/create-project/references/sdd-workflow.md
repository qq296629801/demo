...
| 功能规格 | 单个业务能力的需求与契约                              | `specs/modules/<moduleName>/requirements.md`                            |
| 后端规格 | IIDP 后端技术落地细节（模型、服务、视图、菜单、权限） | `specs/modules/<moduleName>/backend-spec.md`                            |
| 前端规格 | IIDP 前端技术落地细节（§9 实现分支决定是否写代码）         | `specs/modules/<moduleName>/frontend-spec.md`                           |
| 实现计划 | 文件清单、实现顺序、风险                              | `specs/modules/<moduleName>/plan.md`                                    |
| 任务清单 | 可逐项执行的工作项                                    | `specs/modules/<moduleName>/tasks.md`                                   |
| 验收清单 | 静态、构建、运行、页面检查                            | `specs/modules/<moduleName>/validation.md`                              |
...
按 `sdd-constitution.md § requirements.md 模板` 填写，生成包含以下六节的完整文档：

| 章节 | 内容 | 对应 spec-kit |
|---|---|---|
| `## 能力识别` | 模型清单、跨模型服务、后端能力域、前端实现分支意向 | Key Entities |
| `## 用户故事与验收场景` | US-01… P1/P2/P3 优先级；Given/When/Then 验收场景；边界条件 | User Scenarios & Testing |
| `## 功能需求` | FR-001… 编号需求列表；后端五问（用户要完成什么/后端要提供什么/前端分支/契约/验收） | Functional Requirements |
| `## 成功标准` | SC-001… 可量化业务与性能指标 | Success Criteria |
| `## 假设与边界` | 用户假设、范围边界、明确不做事项、外部依赖 | Assumptions |
| `## Clarifications` | 留空，由 Step 1.2 Clarify 写入 | — |
...
```text
请对 specs/modules/{moduleName}/requirements.md 做 Clarify 扫描：
...
**② contracts.md** — API 级契约（每个 Feature 生成 `specs/modules/<moduleName>/contracts.md`）
...
- [ ] [S] 从主干创建 feature 分支：`git checkout -b feature/{appName}/{moduleName}`
- [ ] [S] 确认切换成功后再开始任何文件改动
...
## 前端·代码生成协议门禁（需要前端代码时必做）

- [ ] [S] 读取 `skills/frontend/references/iidp-frontend-codegen-protocol.md`
- [ ] [S] 执行实现前门禁：工程与应用、契约、selector 来源、组件规则、待确认项
- [ ] [S] 门禁失败时停止写代码，先修正 `frontend-spec.md`、`contracts.md` 或待确认项
...
- [ ] [S] 执行 `iidp-frontend-codegen-protocol.md` 的实现后合规扫描
...
| 前端规格 → 代码（含工程初始化）        | `iidp-frontend`（`skills/frontend/SKILL.md`）路由 → `iidp-frontend-spec-code`      | `frontend-spec.md`（必须存在） |
| 前端扩展开发（已有工程内）             | `iidp-frontend`（`skills/frontend/SKILL.md`）路由 → `iidp-frontend-extension-dev` | `frontend-spec.md`             |
| 框架/组件/扩展协议查阅                 | `iidp-frontend`（`skills/frontend/SKILL.md`）路由 → `iidp-frontend-dev-manual`     | 无                             |
| 节点 id 后缀推导                       | `iidp-frontend`（`skills/frontend/SKILL.md`）路由 → `iidp-frontend-standard-ids`   | 无                             |
...
- 前端写代码前必须读取并执行 `skills/frontend/references/iidp-frontend-codegen-protocol.md`；生成或修改 IIDP 组件节点前必须先读取 `iidp-frontend-extension-dev/COMPONENT_RULES_COVERAGE.md`，再读取 `iidp-frontend-extension-dev/COMPONENT_RULES.md` 或 dev-manual，不得用 Element UI、HTML 或通用 Vue 经验补写平台组件属性。
- 前端任务完成前必须执行 codegen protocol 的实现后合规扫描；扫描失败不得勾选任务完成。
...
> 以下命令与上方自动工作流完全独立，不影响自动流程。  
> 将 `skills/create-project/commands/` 下的文件复制到 `.claude/commands/` 后，每个步骤可单独手动触发。
...
安装后在 Claude Code 中输入 `/sdd-` 即可看到所有可用命令。

### 命令速查表

| 命令 | 对应步骤 | 核心产物 | 下一步 |
|---|---|---|---|
| `/sdd-specify` | Step 0 + 1 | `requirements.md` | `/sdd-clarify` |
| `/sdd-clarify` | Step 1.2 | 更新 `requirements.md § Clarifications` | `/sdd-clarify` 或 `/sdd-contracts` |
| `/sdd-contracts` | Step 1.3a ★强制 | `integration-map.md`、`contracts.md` | `/sdd-critique` 或 `/sdd-spec` |
| `/sdd-critique` | Step 1.4 可选 | Critique 报告（不修改规格） | `/sdd-spec` |
| `/sdd-spec` | Step 1.5 | `backend-spec.md`、`frontend-spec.md` | `/sdd-plan` |
| `/sdd-plan` | Step 2 + Gate | `plan.md`、更新 `CLAUDE.md` | `/sdd-tasks` |
| `/sdd-tasks` | Step 3 | `tasks.md`、`validation.md` TC 节 | `/sdd-blueprint` 或 `/sdd-implement` |
| `/sdd-blueprint` | Step 3.5 可选 | 代码蓝图（伪代码 + 文件清单） | `/sdd-implement` |
| `/sdd-implement` | Step 4（单任务） | 一个任务的代码变更，勾选 tasks.md | 重复直到全完成；brownfield 自动交接 `/sdd-sync`，非 brownfield 自动交接 `/sdd-validate` |
| `/sdd-sync` | Step 4.5（同步） | 规格同步；brownfield 刷新 `codebook/baseline-spec/`、`specs/baseline/` 和 `final-diff.md` | 无阻塞时自动交接 `/sdd-validate` |
| `/sdd-validate` | Step 5 | 验收报告，更新 `validation.md` TC 状态 | `/sdd-review` |
| `/sdd-review` | Phase 结束 | 5 子 Agent 深度审查报告，更新 `CLAUDE.md` | `/sdd-specify`（下一 Phase）|

### handoffs 说明

每个命令文件的 frontmatter 包含 `handoffs` 字段，声明完成后的推荐下一步：

```yaml
handoffs:
  - label: 下一步名称          # 显示给用户的选项文字
    command: sdd-next          # 对应的下一个命令
    prompt: 继续提示文字        # 自动衔接时传给下一命令的上下文
    send: true                 # true = 自动衔接；false = 仅提示，等用户手动触发
```

`send: true` 的 handoff 在命令完成且无阻塞项时自动衔接下一步；只有遇到权限请求、失败项、待确认决策或命令明确要求用户确认时才暂停。
`send: false` 的 handoff 仅作为可选路径展示，不自动触发。

IIDP brownfield 收尾链路必须使用自动衔接：`/sdd-implement` 全部任务完成后自动进入 `/sdd-sync`，`/sdd-sync` 成功刷新 baseline 且 `final-diff.md` 无阻塞项后自动进入 `/sdd-validate`。

### CLAUDE.md 标记约定

`/sdd-plan` 和 `/sdd-review` 完成后会写回 `CLAUDE.md` 的标记区间，记录当前活动模组和阶段：
...
新 Claude 会话启动时读取此标记即可续接上次进度，无需重新解释上下文。
