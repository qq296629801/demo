...
- 前端写代码前必须读取 `iidp-frontend-extension-dev` 的扩展协议；生成或修改 IIDP 组件节点前必须读取 `iidp-frontend-extension-dev/COMPONENT_RULES.md`，不得用 Element UI、HTML 或通用 Vue 经验补写平台组件属性。
...
| `/sdd-implement` | Step 4（单任务） | 一个任务的代码变更，勾选 tasks.md | 重复直到全完成，再 `/sdd-validate` |
...
`send: true` 的 handoff 在命令完成后会自动提示运行下一步（等用户确认）；  
