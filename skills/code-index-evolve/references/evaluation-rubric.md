...
`接口文档完整性` 和 `可追溯性与无幻觉` 是主权重；Agent 评审验证文档结构规范性；
变量完整性和文件完整性是底线门控。

| 维度 | 分值 | 衡量内容 |
|---|---:|---|
| 接口文档完整性 | 30 | api.md 接口数 vs ground truth；实体/模块覆盖率 |
| 可追溯性与无幻觉 | 25 | 文档声明能否回溯到源码；无虚构字段/错误码 |
| Agent 评审质量 | 25 | 三个专家 agent 对 hla/api/prd 的结构打分 |
...
- 5 个错误码验证（每错误码 1 分）：
  - 编号和常量名完全一致 → 1 分
  - 常量名一致但编号不同 → 0.5 分（注解推断误差）
  - 找不到该常量 → 0 分

- 无虚构错误码（5 分）：
  - api.md 中所有错误码均能在源码中找到 → 5 分
  - 每有 1 个虚构错误码扣 1 分，最低 0 分
...
通过三个专家 agent 对核心文档进行结构化评审。详细评审方式见
...
| Agent | 审查文档 | 满分 |
|---|---|---|
| `testing-reality-checker` | 每个模块的 `hla.md` | 10 |
| `testing-api-tester` | 每个模块的 `api.md` | 8 |
| `product-manager` | 每个模块的 `prd.md` | 7 |

**多模块处理：** 对每个模块分别评审，取平均分后按满分折算。
例如：3 个模块的 reality-checker 评分分别为 7/10、8/10、6/10 → 平均 7/10 → 最终得 7 分。
