...
## 系统级文件
- [ ] spec/hla.md
- [ ] spec/database-overview.md
- [ ] spec/overview.md

## 模块级文件
- [ ] spec/modules/{{MODULE}}/overview.md
- [ ] spec/modules/{{MODULE}}/hla.md
- [ ] spec/modules/{{MODULE}}/srs.md
- [ ] spec/modules/{{MODULE}}/prd.md
- [ ] spec/modules/{{MODULE}}/user-stories.md
- [ ] spec/modules/{{MODULE}}/api.md
- [ ] spec/modules/{{MODULE}}/database.md
- [ ] spec/modules/{{MODULE}}/error-codes.md
- [ ] spec/modules/{{MODULE}}/flowcharts/
- [ ] spec/modules/{{MODULE}}/ui/
```
...
→ 在 spec/overview.md 中注明"技术栈未能自动识别"
...
> 本文件是 Spec 输出结构与断点恢复契约的唯一来源，不再维护独立的 `spec-templates.md`。
...
所有项目统一采用模块化输出目录：系统级文档放在 `spec/` 顶层，模块级详细设计放在 `spec/modules/{module}/`。

# Phase B 完成后立即写入进度文件（断点恢复的基础）
Write("spec/.progress.md") → 写入完整任务清单（见本文件"断点恢复"章节的 `.progress.md` 最小模板）
...
CF3. API 服务层
     ...
     → 作为 {{FRONTEND_API_SOURCE}} 变量注入 Prompt 5（spec/modules/{module}/api.md 前端调用对照）
...
# 流程图生成规则（flowcharts/）：
# for each endpoint in ENDPOINT_LIST where method in [POST, PUT, DELETE, PATCH]：
#   生成 spec/modules/{module}/flowcharts/{operation}-{entity}-flow.mmd（用 C3 trace 数据驱动）
