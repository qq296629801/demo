...
> **原则**：本表只负责"识别框架是什么"，不涉及 Codebook 输出路径（输出路径由 SKILL.md 工作流决定）。
...
codebook/modules/{module}/hla.md（Redux/Zustand）：
  Store 名称 | State 字段（字段名/类型/初始值）| Actions 列表

codebook/modules/{module}/api.md（"前端调用层对照"章节，由 Prompt 7 生成）：
  函数名 | HTTP 方法 | 端点路径 | TS 请求类型 | TS 响应类型 | 使用的页面
  （前端 API 调用层内容已并入模块 API 文档同一接口下）
```
