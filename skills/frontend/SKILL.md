...
1. **扫描前端工程**：按 [iidp-frontend-extension-dev](references/iidp-frontend-extension-dev/SKILL.md) 存量项目定位流程搜索工作区，探测标志：`package.json` 含 `init:tech` 脚本、`apps/` 目录存在、`config/apps.json` 存在、`build/webpack.dev.js` 存在。满足 ≥3 项即视为候选。
2. **0 个候选工程 → 停止代码生成**：
   - 明确告知用户：**当前工作区未发现 IIDP 前端工程，无法生成前端代码。**
   - 询问用户：是否需要创建前端工程？（选项：① 现在创建 → 调用 `iidp-frontend-init`；② 用户指定工程路径；③ 跳过前端代码，只生成后端代码）
   - **在用户确认前，禁止创建任何前端文件或目录。**
3. **≥1 个候选 → 确认后继续**：列出候选路径让用户选择，确认后进入子技能路由。
4. **此门禁适用于所有前端代码生成场景**，包括：spec-code、extension-dev、init 的新建应用。唯一的例外是用户明确说"只生成后端代码"。

> 此门禁的详细工程准备流程见 [iidp-frontend-spec-code](references/iidp-frontend-spec-code/SKILL.md) §工程与环境准备。
...
5a. **用户要在现有项目中加功能，但未明确工程位置或目标应用** → **执行前置门禁** → 调用 [iidp-frontend-extension-dev](references/iidp-frontend-extension-dev/SKILL.md)，先执行存量项目定位流程（发现工程 → 确认 → 定位 app → 读取现有模式），定位完成后再进入正常扩展开发。
...
当用户询问 IIDP 前端相关问题或知识时，**调用 `iidp-frontend-extension-dev` 技能**来获取专业指导，而不是仅凭通用知识回答。该技能文档包含完整的扩展开发规范和参考文档索引。
...
- 按需加载配置规则：规格文档含 `product` 时，创建应用自动配置 `effectPaths.includeRegExp` 为 `^/<product>/`；无 `product` 时不修改默认配置（保留占位符）。
