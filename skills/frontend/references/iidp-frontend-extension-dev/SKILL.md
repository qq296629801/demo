...
description: 协助在已有 IIDP 前端工程和应用中开发业务功能扩展。当用户询问 IIDP 前端相关问题或知识，或需要实现 IIDP 前端业务功能、写应用扩展、扩展视图（普通节点扩展和扩展钩子）、修改节点、配置数据源、属性绑定、选择器、事件绑定、公共命令时使用。
---
...
如果当前任务会写入或修改前端代码、文件或配置，必须先执行 [iidp-frontend-codegen-protocol](../iidp-frontend-codegen-protocol.md)。本 skill 只提供扩展开发机制和平台事实，不替代代码生成协议。

### 存量项目定位流程

当用户未指定工程路径或 `appName` 时，按 [iidp-frontend-project-locator](../iidp-frontend-project-locator.md) 定位 IIDP 前端工程、扩展应用和现有代码模式。

如果用户已明确提供工程路径和 `appName`，验证路径存在后直接进入下方"开始实现前先确认"。
...
- 定位：在既有 IIDP 前端工程与应用内实现业务扩展（扩展视图〔普通节点扩展和扩展钩子〕/数据源/绑定/事件/commands/自定义组件）。
- 上游：[iidp-frontend-codegen-protocol](../iidp-frontend-codegen-protocol.md)（代码生成强制协议）、[iidp-frontend-dev-manual](../iidp-frontend-dev-manual/SKILL.md)（框架机制、组件属性、扩展协议、工程结构等事实参考）。
- 下游：通常被 [iidp-frontend-spec-code](../iidp-frontend-spec-code/SKILL.md) 调用，用于将规格落到扩展实现；也可直接承接业务开发任务。

## 工程结构定位

> 如果工程根目录和 appName 尚未确定，先执行 [iidp-frontend-project-locator](../iidp-frontend-project-locator.md)。
...
不要把业务代码写进运行临时生成目录或编译产物：`dist`、`distApp`、`distTmp`、`umdComps`、`build`。如果仓库中存在源目录和生成目录同名，先结合构建脚本、入口导入和 git 状态判断真实源文件。
...
> **遇到框架能力不确定时（数据源类型、bind_ 属性名、事件名、vm.biz API、goTo 方法等），必须先读取 [iidp-frontend-dev-manual](../iidp-frontend-dev-manual/SKILL.md)**，不得凭示例或经验猜测。该 skill 涵盖节点系统、数据源、属性绑定、选择器、数据联动、事件绑定、公共命令、视图行为协议、扩展钩子、扩展开发等核心机制的完整参考，以及所有组件的文档索引。需要原始文档细节时再按索引到 `iidpDoc/` 查找。

## 组件规则

生成或修改 IIDP 组件节点前，按以下优先级查找组件规则：

1. 先读取 [COMPONENT_RULES_COVERAGE.md](COMPONENT_RULES_COVERAGE.md)，确认目标组件是否已覆盖。
2. 已覆盖的组件再读取 [COMPONENT_RULES.md](COMPONENT_RULES.md)，并按该文件写法生成。
3. COMPONENT_RULES.md 未覆盖的组件，先通过 [iidp-frontend-dev-manual](../iidp-frontend-dev-manual/SKILL.md) 到 IIDP 开发文档中查找。
4. 文档中也找不到时，询问用户补充组件文档或确认属性，不要猜。
...
标准页生命周期、查询、保存、权限控制等业务逻辑优先使用扩展视图中的扩展钩子；单纯节点结构调整再使用普通节点扩展。

扩展钩子通常挂在当前页面顶级节点上：`页面菜单 url + _container_main`，例如 `rbac_user_app_menu_container_main`。

### 扩展钩子文件位置与格式

**扩展钩子写在 `apps/<appName>/views/<business>/<business>.js`**，与普通节点扩展放在一起，由 `views/index.js` 汇总导出。**不要写到 `common/hook.js`**。

**正确格式**：扩展钩子必须使用 `selector` + `type: "merge"` + `hook` 结构：
...
执行以下流程前，若会写代码，先完成 [iidp-frontend-codegen-protocol](../iidp-frontend-codegen-protocol.md) 的实现前门禁。

1. 定位工程与应用：如果工程路径和 appName 尚未确定，先执行 [iidp-frontend-project-locator](../iidp-frontend-project-locator.md)。确认后，读取 `apps/<appName>/views/index.js`、已有业务文件命名风格、`config/app.json` 中的按需加载配置。
2. 理解现有视图：找目标页面、节点 id、数据源名、绑定表达式、事件和 commands；必要时用 `tech_app.printObj(...)` 的思路指导用户在浏览器中查看节点属性。
3. 判断扩展方式：生命周期、查询、保存、权限控制优先用扩展视图中的扩展钩子；节点结构调整、属性合并、插入删除再用普通节点扩展。
4. 确认组件协议：新增或修改组件节点前先读取 `COMPONENT_RULES_COVERAGE.md`，再读取 `COMPONENT_RULES.md`；目标组件未覆盖时查 dev-manual，仍无文档则先询问用户，不猜属性。
...
8. 执行 codegen protocol 的实现后合规扫描。
9. 验证结果：优先运行项目已有 lint/test/build；无法运行时说明应在页面上验证的节点、事件、接口请求和数据变化。
...
- 使用扩展钩子时给出 hook 路径、是否调用 `vm.super`、关键 `vm.biz` 读写点。
