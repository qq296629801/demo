---
description: 【Evolve Tool】通过 Playwright MCP 访问开源网站 Demo，截图并分析功能，与 IIDP 生成规格/代码对比，输出合规评分报告（V1-V10，10维，100分）。注意：本文件的 V1-V10 是 OSS 合规维度，与 skill-rubric.md 的 D1-D10（skill 诊断维度）完全无关。
---
...
### 步骤 3：对比分析（10 维 OSS 合规评分，V1-V10）

> **命名说明**：以下维度编号为 V1-V10（OSS Validation），与 `skill-rubric.md` 的 D1-D10（Skill Diagnostic）完全无关，不要混用。

**对比 OSS 功能 vs IIDP 生成规格，计算各维度得分：**

| 维度 | 计算方式 | 满分 |
|---|---|---|
| V1 功能覆盖率 | IIDP 功能数 / OSS 功能数 × 20 | 20 |
| V2 页面完整性 | list✅+form✅+detail×(1-缺失率)+workflow×(1-缺失率) | 15 |
| V3 字段完整性 | IIDP 字段数 / OSS 可见字段数 × 15 | 15 |
| V4 操作完整性 | IIDP 按钮数 / OSS 按钮数 × 15 | 15 |
| V5 状态流转 | 状态枚举覆盖率 × 10 | 10 |
| V6 业务规则 | 必填/校验覆盖率 × 10 | 10 |
| V7 权限粒度 | 有按钮级权限控制 → 5，否则 → 0 | 5 |
| V8 接口规范 | 分页/排序/Filter 支持 → 5，否则 → 0 | 5 |
| V9 数据关联 | ER 关系数 / OSS 表单关联数 × 3 | 3 |
| V10 错误处理 | 有错误码/友好提示声明 → 2，否则 → 0 | 2 |
...
```
Gap 识别规则：
- V2 页面完整性 < 10 且 detail_exists=false → G4-1（缺少详情页）
- V2 页面完整性 < 10 且 workflow_exists=false 但 OSS 有审批 → G5-2（缺少审批服务）
- V5 状态流转 < 7 → G5-1（状态枚举不完整）
- V7 权限粒度 = 0 → G4-2（按钮权限缺失）
- V8 接口规范 = 0 → G8-2（缺 Filter/Sort 支持）
- V3 字段完整性 < 10 → G7-1（字段校验缺失）
```
